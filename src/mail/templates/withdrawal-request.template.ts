...
export function buildWithdrawalRequestEmailTemplate(
  brandName: string,
  ...
  const narration = withdrawalRequest.narration || 'Wallet withdrawal request';
  const requestId = withdrawalRequest.id?.substring(0, 8).toUpperCase() || 'N/A';
  const createdAt = withdrawalRequest.createdAt
    ? new Date(withdrawalRequest.createdAt).toLocaleString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
  ...
  const preheader = `Your withdrawal request of ${amount} has been submitted and is pending admin approval.`;
...
<head>
  ...
  <style>
    ...
    .amount-box { background:#eff6ff; border:2px solid #93c5fd; border-radius:8px; padding:16px; text-align:center; margin:24px 0; }
    .amount-label { font-size:12px; color:#1e40af; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px; font-weight:600; }
    .amount-value { font-size:28px; font-weight:700; color:#1e40af; }
        ...
        <table role="presentation" class="card" cellspacing="0" cellpadding="0">
          <tr>
            <td class="header">
              ...
              <div class="header-subtitle">Withdrawal Request Submitted</div>
            </td>
          </tr>
          <tr>
            <td class="content">
              <h1 class="title">Withdrawal Request Received 💰</h1>
              <p class="subtitle">Your withdrawal request has been submitted and is pending admin approval.</p>
              
              <table role="presentation" class="amount-box" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center">
              ...
              <table role="presentation" class="info-table" cellspacing="0" cellpadding="0">
                ...
                <tr class="info-row">
                  ...
                  <td class="info-value" width="50%" align="right"><strong>Pending Approval</strong></td>
                </tr>
                <tr class="info-row info-row-last">
                  <td class="info-label" width="50%">Submitted At</td>
                  <td class="info-value" width="50%" align="right">${createdAt}</td>
                </tr>
              </table>

              ${narration ? `
              <div style="margin:24px 0;">
                <div style="font-weight:600; color:#475569; font-size:14px; margin-bottom:8px;">Description</div>
                <div style="color:#0f172a; font-size:14px; line-height:1.6;">${narration}</div>
              </div>
              ` : ''}
              ...
              <table role="presentation" class="highlight-box" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td>
                    ...
                    Your withdrawal request is now under review by our admin team. You will receive an email notification once your request has been approved or rejected. This process typically takes 24-48 hours.</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td class="footer">
              You're receiving this email because a withdrawal request was submitted from your ${brandName} wallet account.<br>
...
What happens next?
Your withdrawal request is now under review by our admin team. You will receive an email notification once your request has been approved or rejected. This process typically takes 24-48 hours.

You're receiving this email because a withdrawal request was submitted from your ${brandName} wallet account.
If you did not make this request, please contact our support team immediately.`;
